...
def start_codon_finder(dna_seq):
    """
    :param dna_seq: A string of DNA sequence
    :return: A list contains the position of first base in start codons
    """
    start_codons = []
...
def stop_codon_finder(dna_seq):
    """
    :param dna_seq: A string of DNA sequence
    :return: A list contains the position of first base in stop codons
    """
    stop_codons = []
    for i in range(len(dna_seq) - 2):
        codon = dna_seq[i:i+3]
...
def orf_finder(dna_seq):
    """
    :param dna_seq: A string of DNA sequence
    :return: A string of the longest possible open reading frame
    """
    orf_seq = ""
    ...
    if len(positive_starts) == 0 and len(negative_starts) == 0:
        print("No start codon found both on positive strand and negative strand")
        exit()
    if len(positive_stops) == 0 and len(negative_stops) == 0:
        print("No stop codon found both on positive strand and negative strand")
        exit()
    if len(positive_starts) == 0 and len(negative_stops) == 0:
        print("No start codon found on positive strand and no stop codon on negative strand")
        exit()
    if len(positive_stops) == 0 and len(negative_starts) == 0:
        print("No stop codon found on positive strand and no start codon on negative strand")
        exit()
...
def translate(orf):
    """
    :param orf: String of open reading frame sequence
    :return: String of translation of ORF to amino acid sequences
    """
    amino_acids = {
